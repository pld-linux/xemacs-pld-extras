...
(defun rpm-build (buildoptions)
  ...
  (goto-char (point-max) (get-buffer rpm-buffer-name))
  (let ((process
...
(defun rpm-spec-initialize ()
  ...
  (let (file name version (release "1"))
    (setq file (if (buffer-file-name)
		 ...
		 (buffer-name)))
    ;; try to find version number and release from the filename
    (cond
    ...
    (insert "# " file
	    ...
	    "\nRelease: " (or release "")
	    "\nCopyright: \nGroup: \n\n%description\n"
	    (if rpm-initialize-sections
		"\n%changelog\n%prep\n%setup\n\n%build\n\n%install\n\n%files\n"
	      "")
	    "\n# end of file\n")
    (rpm-add-change-log-entry "initialization of spec file.\n")))
